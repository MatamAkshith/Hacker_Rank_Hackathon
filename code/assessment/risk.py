from code.context.models import UnifiedContext
from code.understanding.models import UnderstandingResult
from code.assessment.models import RiskAssessment

class RiskCalculator:
    """Calculates security, scam, and spam risk values from message context and understanding."""
    
    def calculate(self, context: UnifiedContext, understanding: UnderstandingResult) -> RiskAssessment:
        """Returns default RiskAssessment."""
        return RiskAssessment()
