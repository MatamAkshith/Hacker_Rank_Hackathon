"""GeminiApiClient: Client wrapper for Google's native Gemini API.

Extracted from the original gemini_client.py.
"""
...
class GeminiApiClient:
    ...
    def _pace_requests(self) -> None:
        ...
        elapsed = time.time() - GeminiApiClient._last_request_at
        wait_for = self._min_request_interval_seconds - elapsed
        if wait_for > 0:
            time.sleep(wait_for)

    def _active_models(self) -> List[str]:
        return [m for m in self.model_chain if m not in GeminiApiClient._exhausted_models]
            ...
            for attempt in range(max_attempts):
                ...
                try:
                    ...
                    with urllib.request.urlopen(req, timeout=120) as response:
                        GeminiApiClient._last_request_at = time.time()
                    ...
                    if e.code == 429:
                        if _is_daily_quota_error(error_body):
                            ...
                            GeminiApiClient._exhausted_models.add(model_name)
